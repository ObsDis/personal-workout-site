...
export function buildServer(db: SupabaseClient, user: { id: string; email?: string }) {
  ...
  server.registerTool(
    'set_profile',
    { title: 'Set profile', description: 'Create or update the user profile in a single upsert (works for a brand-new account with no profile row). Used by the onboarding form on submit, and callable directly. Fields: name, height_ft, height_in, weight_lbs, goal (build_muscle/lose_weight/gain_strength/maintain/general_fitness), kcal_target, protein_target_g, training_split, injury_notes. Only provided fields change.', inputSchema: { name: z.string().optional(), height_ft: z.number().int().optional(), height_in: z.number().int().optional(), weight_lbs: z.number().optional(), goal: z.enum(['build_muscle', 'lose_weight', 'gain_strength', 'maintain', 'general_fitness']).optional(), kcal_target: z.number().int().optional(), protein_target_g: z.number().int().optional(), training_split: z.string().optional(), injury_notes: z.string().optional() } },
    async (a) => {
      const patch: any = { id: uid, updated_at: new Date().toISOString() }
